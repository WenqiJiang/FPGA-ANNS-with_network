...
void split_cell_ID(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        dist_cell_ID_t tmp;
        int searched_cell_id_local[NPROBE_MAX];
...
void lookup_center_vectors(
    ...
    ap_uint<64> center_vector_local[NLIST_MAX * D / 2];
...
void scan_controller(
    ...
    int start_addr_LUT[NLIST_MAX];
    int scanned_entries_every_cell_LUT[NLIST_MAX];
    int last_valid_channel_LUT[NLIST_MAX];  
